...
from .utils import send_otp_to_user, round_robin_agent_assignment, load_balanced_assignment, skill_based_assignment
...
class LeadViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response({'Lead': serializer.data, "Agent": agent, 'message': f'Lead assigned to {agent.profile.first_name}' if agent else "No agent available"}, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def reassign(self, request, pk=None):
        ...
        try:
            ...
            agent.total_leads += 1
            agent.save()
            return Response({"message": f"Lead re-assigned to {agent.profile.first_name}", "Lead": self.get_serializer(lead).data}, status=status.HTTP_200_OK)
